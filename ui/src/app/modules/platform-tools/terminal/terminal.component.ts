...
export class TerminalComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public ngOnInit() {
    ...
    // If terminal is already ready, check if we should continue with the existing session
    if (this.$terminal.isTerminalReady()) {
      // If persistence is disabled but there's a session, destroy it and start fresh
      if (!this.$settings.terminalPersistence && this.$terminal.hasActiveSession()) {
        ...
        // Otherwise, reconnect to the existing session
